...
## Living Literature Evidence Sequence
1. Recheck drift-prone hardware, disk, memory pressure, swap, licenses, and upstream revisions.
2. Keep dependency/source download, model-weight download, and inference as separate recorded operations.
3. Stop inference if memory pressure, swap growth, thermals, or disk use becomes unsafe.
4. Record only commands that actually ran and distinguish readiness, generation, offline, quality, rights, and publication evidence.
5. Update Obsidian only after repo-local canon is correct; repo-local canon remains authoritative.

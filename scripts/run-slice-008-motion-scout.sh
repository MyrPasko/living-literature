...
output_path="$repo_root/outputs/slice-008/wan2.2-i2v-a14b-first-last-motion-scout-seed-42.mp4"
review_root="$repo_root/outputs/slice-008/review"
contact_sheet="$review_root/contact-sheet.png"
artifact_root="$repo_root/benchmarks/artifacts/slice-008"
...
if [[ "$(git -C "$repo_root" branch --show-current)" != "feature/slice-008-corrected-first-last-motion-scout" ]]; then
  printf 'Refusing scout: wrong branch.\n' >&2
  exit 1
fi
if [[ -n "$(git -C "$repo_root" status --porcelain=v1)" ]]; then
  printf 'Refusing scout: the tracked working tree is not clean.\n' >&2
  git -C "$repo_root" status --short >&2
  exit 1
fi
if ! git -C "$repo_root" merge-base --is-ancestor 4bf7493 HEAD || \
   ! git -C "$repo_root" merge-base --is-ancestor 39d3ad3fbc965d70a267c20fb7e57cc839769a79 HEAD; then
  printf 'Refusing scout: required preparation or merged baseline commit is not an ancestor.\n' >&2
  exit 1
fi
if pgrep -fl 'mlxgen.*generate|mlxgen-generate-wan|mflux.models.wan.cli.wan_generate|ComfyUI|diffusers.*(generate|inference)' >/dev/null 2>&1; then
  printf 'Refusing scout: another model-inference process is running.\n' >&2
  pgrep -fl 'mlxgen.*generate|mlxgen-generate-wan|mflux.models.wan.cli.wan_generate|ComfyUI|diffusers.*(generate|inference)' >&2 || true
  exit 1
fi

for tracked_path in \
  benchmarks/slice-008-corrected-first-last-motion-scout.json \
  docs/slice-008-corrected-first-last-motion-scout.md \
  ...
  scripts/run-slice-008-motion-scout.sh \
  scripts/verify-slice-008.py \
...
"$project_python" "$repo_root/scripts/verify-slice-008.py" --preflight
"$project_python" "$repo_root/scripts/verify-slice-007.py" --preflight

if [[ ! -x "$runtime_cli" ]]; then
  printf 'Refusing scout: missing pinned direct Wan CLI at %s.\n' "$runtime_cli" >&2
  exit 1
fi
...
if ! "$runtime_cli" --help | grep -q -- '--last-image'; then
  printf 'Refusing scout: direct Wan CLI does not expose --last-image.\n' >&2
  exit 1
fi
...
if ! (set -o noclobber; printf 'started_at_utc=%s\ngit_commit=%s\n' "$started_at" "$git_commit" >"$attempt_marker"); then
  printf 'Refusing scout: could not atomically create the one-attempt marker.\n' >&2
  exit 1
fi
...
printf 'run_id=%s\nmode=local-files-only\nstarted_at_utc=%s\noutput=%s\nmetadata=%s\nfirst_image=%s\nlast_image=%s\nmodel_snapshot=%s\nruntime_root=%s\ngit_commit=%s\n' \
  "$run_id" "$started_at" "$output_path" "$metadata_path" "$first_image" "$last_image" \
  "$model_snapshot" "$runtime_root" "$git_commit" >"$summary_path"
...
while kill -0 "$run_pid" 2>/dev/null; do
  ...
  if ((current_denoise_count > denoise_event_count)); then
    ...
    latest_denoise="$(grep '"phase": "denoise"' "$log_path" | tail -n 1)"
...
printf 'ended_at_utc=%s\nduration_seconds=%s\ninference_exit_code=%s\nsafety_stop=%s\ndenoising_started=%s\ndenoise_event_count=%s\n' \
  "$ended_at" "$duration_seconds" "$exit_code" "$safety_stop" "$denoising_started" \
  "$denoise_event_count" >>"$summary_path"
...
ffmpeg -v error -n -i "$output_path" \
  -vf "select='eq(n,0)+eq(n,8)+eq(n,16)+eq(n,24)+eq(n,32)+eq(n,40)',scale=448:256,tile=3x2:padding=8:margin=8" \
  -frames:v 1 -fps_mode vfr "$contact_sheet"

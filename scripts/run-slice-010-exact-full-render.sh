...
output_path="$repo_root/outputs/slice-010/wan2.2-i2v-a14b-exact-first-last-full-render-seed-42.mp4"
review_root="$repo_root/outputs/slice-010/review"
...
if [[ "$(git -C "$repo_root" branch --show-current)" != "feature/slice-010-wan2-2-exact-full-render" ]]; then
...
if ! git -C "$repo_root" merge-base --is-ancestor 96a1ac2f9dbad2e99e120ae8606edd49fe2019ca HEAD; then
  printf 'Refusing full render: completed Slice 009 is not an ancestor.\n' >&2
  exit 1
fi
...
for tracked_path in \
  benchmarks/slice-010-wan2.2-exact-first-last-full-render.json \
  docs/slice-010-wan2.2-exact-first-last-full-render.md \
  ...
  scripts/run-slice-010-exact-full-render.sh \
  scripts/verify-slice-010.py \
...
"$project_python" "$repo_root/scripts/verify-slice-010.py" --preflight

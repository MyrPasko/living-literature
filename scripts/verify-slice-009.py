...
BENCHMARK_ID = "slice-009-wan2.2-first-last-full-render"
RUN_ID = "local-files-only-first-last-full-render-seed-42"
BASELINE_COMMIT = "6a3e3381b3190c348697557f60ed05b257c60115"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify Slice 009 configuration and evidence.")
...
def verify_configuration(repo_root: Path, benchmark: dict) -> None:
    ...
    runner = (repo_root / "scripts/run-slice-009-full-render.sh").read_text(encoding="utf-8")
    for value in (benchmark["prompt"], benchmark["negative_prompt"], "--last-image", "--width 832", "--height 480", "--frames 81", "--fps 16", "--steps 50", "--guidance 5.0", "--guidance-2 5.0", "--flow-shift 5.0", "--no-replace"):
...
def verify_preflight(repo_root: Path) -> None:
    require(command_output(["git", "-C", str(repo_root), "branch", "--show-current"]).strip() == "feature/slice-009-wan2-2-first-last-full-render", "wrong branch")
...
def verify_result(repo_root: Path, benchmark: dict, allow_pending_review: bool) -> None:
    result_path = repo_root / "manifests/benchmarks/slice-009-wan2.2-first-last-full-render-results.json"
...
def main() -> int:
    ...
    benchmark = load_json(repo_root / "benchmarks/slice-009-wan2.2-first-last-full-render.json")
    ...
    if args.configuration_only:
        ...
        print("slice-009-verification=pass state=configuration-only")
        return 0
    if args.preflight:
